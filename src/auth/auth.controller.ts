...
export class AuthController {
    ...
    @Post("reset-password")
    async resetPassword(@Body() dto: ResetForgotPasswordDto, @Res() res: Response){
        console.log(dto)
   ...
   @UseGuards(AuthGuard)
    ...
    async changePassword(@Body() dto: ChangePasswordDto , @Res() res: Response, @Req() req: Request){
        const userId = req.user.id;
        const response = await this.authService.changePassword(userId,dto.oldPassword, dto.newPassword);
        return res.status(response.status).json(response);
    }
}